...
## Discussion 6: Using the SKILL.md Files to Steer Implementation
**Developer:** The assignment requires that we not only create two `SKILL.md` files but also demonstrate how they actually steered the AI tool during the implementation of the web app.
**AI:** The skill files successfully steered our implementation of the web app in two concrete ways:
1. `media-react-data-wiring` explicitly dictated that the React layer must act purely as a data-provider and must not leak Pexels implementation details. This steered me to design `useMediaSearch` and `useMediaFeed` in the web app to share a unified response shape, allowing the presentation layer to remain entirely unaware of the difference between a search query and a curated feed. It also instructed me to initialize the `MediaProvider` with the API key at the root of `App.tsx` rather than passing it down to individual components.
2. `media-ui-react-components` laid down the law that the UI components must be headless and return prop-getters. This strictly steered me away from rendering HTML nodes inside the UI library. When building `apps/web/src/components/MediaGrid.tsx` and `VideoReels.tsx`, I consumed the hooks exactly as documented by the skill (e.g., extracting `getContainerProps` and `getItemProps`) and applied the CSS classes and DOM nodes entirely within the web app context.
